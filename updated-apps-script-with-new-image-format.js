// Updated Google Apps Script with New Image URL Format
// Uses https://drive.usercontent.google.com/download?id= format for better image loading
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (row[0] && row[0].toString().trim() !== '') {
      const property = {
        ...
        images: processPhotoUrls(row[6] || '') // Column G: Photos - NEW FORMAT
      };
...
// Process photo URLs - UPDATED TO USE NEW FORMAT
function processPhotoUrls(photoString) {
  ...
  for (const url of urls) {
    if (url && url.includes('drive.google.com')) {
      ...
      // Convert to new format if file ID was found
      if (fileId) {
